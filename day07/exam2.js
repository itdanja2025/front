...
총합계 += 4;
총합계 += 5; 
// 1.반복되지않는코드 : (1) let 총합계 = 0;  , (2) 1 2 3 4 5 
// 2.반복되는코드 : (3) 총합계 +=
// 3.반복되지않는코드 (증감) 패턴 ? (1)없다. (2) 1부터 5까지 1씩증가.
...
// 1. 반복되는코드 : console.log( ` 2 X ? = ${ 2 * ? }` );
// 2. 반복되지않는코드 : 1 2 3 4 5 6 7 8 9
// 3. 반복도지않는코드 (증감)패턴 : 1부터 9까지 1씩증가.
...
let ary = [ ]; // 빈배열 생성
// let a = prompt( "값 : ");    ary.push( a );  // 변수명.push( 새로운값 ); 배열내자료추가
// let b = prompt( "값 : ");    ary.push( b );  // 변수명.push( 새로운값 ); 배열내자료추가
// let c = prompt( "값 : ");    ary.push( c );  // 변수명.push( 새로운값 ); 배열내자료추가
// vs count가 1부터 count는 3이하 까지 1씩 증가 하면서 값을입력받아 변수에 저장한후에 배열내 입력받은값(변수의값) 추가한다.
