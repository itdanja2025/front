...
function func1( ){ // 매 : x , 리 : x , 처 : 입력받은값을 (스토리지)저장
    // (1) 입력받은 값 을 가져온다. 
...
let visitCount = 1;
    ...
    // (2) 만약에 존재하지 않은 웹스토리지 이면 스토리지 추가한다. 
    if( visit == null ){ // null 이란 ? 자료가 없다 뜻 
        sessionStorage.setItem( 'visit' , visitCount );
    }else{ // (3) 존재하면 스토리지에서 꺼내온값에 +1 한다. < 스토리지 문자열만 저장한다 > 
        visitCount = Number( visit ) + 1 ; 
        // (4) 더한 값을 스토리지 에도 저장한다.
        sessionStorage.setItem( 'visit' , visitCount );
    }
// 어디에
